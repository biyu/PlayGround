...
function decode( legData ) {
    ...
    leg.first.time = _parseTime( firstLeg.arrTime ).toTimeString().slice(0,5);   // currently only need time string
    leg.last = {};
    leg.last.location = lastLeg.coord;
    leg.last.name = lastLeg.name ? lastLeg.name : '';
    leg.last.time = _parseTime( lastLeg.depTime ).toTimeString().slice(0,5);     // currently only need time string
    leg.type = decode_code( legData.type, legData.code ).type;
    leg.code = decode_code( legData.type, legData.code ).code;
    leg.length = _parseLength(legData.length);
    return leg;
}
...
function _parseLength(length) {
    return (length/1000.0).toFixed(1);
}
